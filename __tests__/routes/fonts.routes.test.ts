...
describe("Testing the fonts routes module", () => {
  ...
  test("Calling create fonts route with correct body should return created message and fonts", async () => {
    ...
    const res = await request(app).post(`/api/fonts/`).send(fontsData).set(AuthMock);
    expect(res.body.message).toBe("Fonts created successfully");
    expect(res.body.data).toHaveProperty("headingFontName", fontsData.headingFontName);
    expect(res.body.data).toHaveProperty("parragraphFontName", fontsData.parragraphFontName);
  ...
  test("Calling delete fonts route with correct params should return deleted fonts count", async () => {
    ...
    const response = await request(app).post(`/api/fonts/`).send(fontsData).set(AuthMock);
    const { _id: id } = response.body.data;
    const res = await request(app).delete(`/api/fonts/${id}`).send().set(AuthMock);
  ...
  test("Calling update fonts route with correct params and body should return updated message and new fonts", async () => {
    ...
    const response = await request(app).post(`/api/fonts/`).send(fontsData).set(AuthMock);
    const { _id: id } = response.body.data;
    const res = await request(app).put(`/api/fonts/${id}`).send(updatedFontsData).set(AuthMock);
    expect(res.body.message).toBe("Fonts updated successfully");
    expect(res.body.data).toHaveProperty("headingFontName", updatedFontsData.headingFontName);
    expect(res.body.data).toHaveProperty("parragraphFontName", updatedFontsData.parragraphFontName);
    expect(res.body.data).toHaveProperty("baseSize", updatedFontsData.baseSize);
    expect(res.body.data).toHaveProperty("scaleFactor", updatedFontsData.scaleFactor);
    await request(app).delete(`/api/fonts/${id}`).send().set(AuthMock);
  });
